...
gboolean on_main_window_key_press_event(GtkWidget *widget, GdkEventKey *event, gpointer data) {
    ...
    if (event->keyval >= GDK_KEY_0 && event->keyval <= GDK_KEY_9) {
        char texto[2];
        g_snprintf(texto, 2, "%c", event->keyval);
        on_botao_clicked(NULL, texto);
        return TRUE;
    }
    if (event->keyval == GDK_KEY_comma || event->keyval == GDK_KEY_period) {
...
void usar_estilo() {
    ...
    gtk_css_provider_load_from_path(css_provider, "C:\\msys64\\home\\center music\\projeto3\\estilo_calculadora.css", NULL);
